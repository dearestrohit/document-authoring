...
/**
 * Transformer: sunfeastyippee Dynamic Media / Scene7 images.
 *
 * Source contains Scene7 IS/Image URLs (s7ap1.scene7.com/is/image/...) —
 * verified via migration-work/metadata.json .images.mapping (8 URLs).
 * Converts DM <img> tags into anchors so the DM URL round-trips through
 * markdown; the companion auto-block in scripts/scripts.js rebuilds them
 * into responsive <picture> elements at render time.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and
 * extract <img> references into block cells (cards-recipe / cards-diy);
 * rewriting imgs to anchors earlier would empty those cells.
 */
